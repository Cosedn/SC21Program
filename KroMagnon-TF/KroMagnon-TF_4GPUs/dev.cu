...
__global__ void COMPUTE_AVG_GRAD_ATOMIC(Ratings *r, float *r0, float *grad_U, float *grad_V, float *grad_W, float *avg_grad_U, float *avg_grad_V, float *avg_grad_W, float *num_u, float *num_v, float *num_w, int *random, int *size)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int t, k;
...
__global__ void SVRG_UPDATE(Ratings *r, float *r0, float *U, float *V, float *W, float *grad_U, float *grad_V, float *grad_W, float *avg_grad_U, float *avg_grad_V, float *avg_grad_W, const int *random, int *size)
{
	int i, k, t;
	float e;
	float grad0, grad1;
...
__global__ void COMPUTE_R0(Ratings *r, float *r0, float *U, float *V, float *W, int *size)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int k;
