...
/*Updating factored matrices with Hogwild! lock-freely.*/	
__global__ void SGD_UPDATE(Ratings *r, float *r0, float *U, float *V, float *W, float *grad_U, float *grad_V, float *grad_W, float *avg_grad_U, float *avg_grad_V, float *avg_grad_W, const int *random)
{
	int i, k, t;
	float e;
	float grad0;
